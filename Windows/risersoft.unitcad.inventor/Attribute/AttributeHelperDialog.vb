...
<System.Runtime.InteropServices.ComVisible(False)> Public Class AttributeHelperDialog
    ...
    Private m_DocEvents As Inventor.DocumentEvents
    Private m_InputEvents As Inventor.UserInputEvents
    Private m_LastMousePosition As System.Drawing.Point
    Private m_AddAttributeSetButtonDef As Inventor.ButtonDefinition
        ...
        Try
            For Each attribSet As MyAttributeSet In Entity.AttributeSets
                ...
                ElseIf attribSet.IsEdited Then
                    invAttribSet = Entity.Entity.AttributeSets.item(attribSet.Name)
                    ...
                    For Each attrib As MyAttribute In attribSet
                        ...
                        ElseIf attrib.IsEdited Then
                            Dim invAttrib As Inventor.Attribute = invAttribSet.Item(attrib.Name)
    ...
    Private Sub CleanUp()
        ...
        If Not m_DocEvents Is Nothing Then
            RemoveHandler m_DocEvents.OnChangeSelectSet, AddressOf m_DocEvents_OnChangeSelectSet
            System.Runtime.InteropServices.Marshal.ReleaseComObject(m_DocEvents)
            m_DocEvents = Nothing
        End If

        If Not m_InputEvents Is Nothing Then
            RemoveHandler m_InputEvents.OnActivateCommand, AddressOf m_InputEvents_OnActivateCommand
            RemoveHandler m_InputEvents.OnContextMenu, AddressOf m_InputEvents_OnContextMenu
        ...
        If Not m_AddAttributeSetButtonDef Is Nothing Then
            RemoveHandler m_AddAttributeSetButtonDef.OnExecute, AddressOf m_AddAttributeSetButtonDef_OnExecute
            System.Runtime.InteropServices.Marshal.ReleaseComObject(m_AddAttributeSetButtonDef)
            m_AddAttributeSetButtonDef = Nothing
        End If

        If Not m_InventorDoc Is Nothing Then
            System.Runtime.InteropServices.Marshal.ReleaseComObject(m_InventorDoc)
    ...
    Private Sub AttributeHelperDialog_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If Not Me.InventorDocument Is Nothing Then
                ...
                AddHandler m_AddAttributeSetButtonDef.OnExecute, AddressOf m_AddAttributeSetButtonDef_OnExecute
                ...
                AddHandler m_DocEvents.OnChangeSelectSet, AddressOf m_DocEvents_OnChangeSelectSet
                m_InputEvents = Me.InventorDocument.Parent.CommandManager.UserInputEvents
                AddHandler m_InputEvents.OnActivateCommand, AddressOf m_InputEvents_OnActivateCommand
                AddHandler m_InputEvents.OnContextMenu, AddressOf m_InputEvents_OnContextMenu
    ...
    Private Sub m_InputEvents_OnActivateCommand(ByVal CommandName As String, ByVal Context As Inventor.NameValueMap)
        ...
        If Not (CommandName.Contains("ViewCmd") OrElse CommandName.Contains("WindowCmd")) Then
            m_HighlightSet.Clear()
            Me.Close()
        End If
    End Sub

    Private Sub m_InputEvents_OnContextMenu(ByVal SelectionDevice As Inventor.SelectionDeviceEnum, ByVal AdditionalInfo As Inventor.NameValueMap, ByVal CommandBar As Inventor.CommandBar)
        Try
            ' Check to see if any of the objects in the select set support attributes.
            Dim entityFound As Boolean = False
            Try
                Dim attribSet As Inventor.AttributeSets = Me.InventorDocument.SelectSet.Item(1).AttributeSets
            ...
            For i As Integer = CommandBar.Controls.Count To 1 Step -1
                Dim control As Inventor.CommandBarControl = CommandBar.Controls.Item(i)

                If Not (control.InternalName.Contains("ViewCmd") OrElse control.InternalName.Contains("WindowCmd") OrElse control.InternalName.Contains("AppHowToCmd")) Then
                    control.Delete()
                End If
            Next

            If entityFound Then
                CommandBar.Controls.AddButton(m_AddAttributeSetButtonDef, 1)
            End If
        Catch ex As Exception
        ...
        If attrib.ValueAsFullString <> AttributeValue Then
            ...
            Select Case attrib.AttributeType
                ...
                Case Inventor.ValueTypeEnum.kBooleanType
                    Dim blnValue As Double
                    Try
                        blnValue = CType(AttributeValue, Boolean)
                    Catch ex As Exception
                        MsgBox("The value entered is not a valid Boolean type.")
                        Return False
                    End Try
                    attrib.Value = blnValue
    ...
    Private Function CreateAttribute(ByVal AttributeName As String, ByVal AttributeType As Inventor.ValueTypeEnum, ByVal AttributeValue As String, ByVal EditNode As TreeNode) As Boolean
        ...
        Select Case AttributeType
            ...
            Case Inventor.ValueTypeEnum.kBooleanType
                ...
                attrib = attribSet.Add(AttributeName, Inventor.ValueTypeEnum.kBooleanType, blnValue)
            Case Inventor.ValueTypeEnum.kByteArrayType
                Dim values() As String = AttributeValue.Split(","c)
                Dim newArray() As Byte
                ReDim newArray(values.Length - 1)
    ...
    Private Sub m_AddAttributeSetButtonDef_OnExecute(ByVal Context As Inventor.NameValueMap)
        Try
    ...
    Private Sub AddAttributeToolStripMenuItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles AddAttributeToolStripMenuItem.Click
        Try
            ...
            Do
                ...
                If editDialog.DialogResult = System.Windows.Forms.DialogResult.OK Then
    ...
    Public ReadOnly Property HasChanges() As Boolean
        Get
            For Each entity As MyEntity In Me
                If entity.IsEdited OrElse entity.IsNew Then
                    Return True
                    Exit For
                End If

                For Each attribSet As MyAttributeSet In entity.AttributeSets
                    If attribSet.IsDeleted OrElse attribSet.IsEdited OrElse attribSet.IsNew Then
                        Return True
                        Exit For
                    End If

                    For Each attrib As MyAttribute In attribSet
                        If attrib.IsDeleted OrElse attrib.IsEdited OrElse attrib.IsNew Then
                            Return True
                            Exit For
                        End If
                    Next
...
Friend Class MyAttributeSet
    ...
    Public ReadOnly Property IsEdited() As Boolean
        Get
            ...
            Else
                For Each attrib As MyAttribute In Me
                    If attrib.IsEdited OrElse attrib.IsNew OrElse attrib.IsDeleted Then
                        Return True
                    End If
                Next
            End If
