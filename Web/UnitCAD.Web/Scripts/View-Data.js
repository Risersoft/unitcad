...
var geometryFilter2d = { 'type': 'geometry', 'role': '2d' };
...
function onSuccessDocumentLoadCB(viewerDocument) {
    ...
    geometryItems3d = Autodesk.Viewing.Document.getSubItemsWithProperties(rootItem, geometryFilter3d, true);
    geometryItems2d = Autodesk.Viewing.Document.getSubItemsWithProperties(rootItem, geometryFilter2d, true);

    if (geometryItems3d.length > 0) {

        var item3d = viewerDocument.getViewablePath(geometryItems3d[0]);
    ...
    } else if (geometryItems2d.length > 0) {

        var item2d = viewerDocument.getViewablePath(geometryItems2d[0]);

        // Load the viewable to the viewer
        viewer3D.load(item2d);

        console.log("Loading 2d Geometry from document : " + documentId);
    }
    else {
        console.log("3d/2d Geometry not found in document : " + documentId);
    }
}
